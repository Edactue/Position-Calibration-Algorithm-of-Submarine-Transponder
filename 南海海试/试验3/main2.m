%
% 南海海试数据处理3
% 声线修正：遍历角度，得到时延误差范围内的水平传播距离,等声速声线跟踪
% 定位算法：卡尔曼滤波 + 高斯牛顿迭代法
%
clc,clear,close all
...
for i = 1 : pos_num
    ...
    for con = 1 : size(angal,2)-2
        % 各层传播所用时间
        simt_i = delta_zi ...
                ./(soundspeed_targ_ana(2:size_speed) ...
                .* sqrt(1-(p(con) .* soundspeed_targ_ana(2:size_speed)).^2));

        sum_t = [sum_t sum(simt_i)]; % 记录各角度到目标深度传播时间
    end
    ...
    for v = 1:5
        ...
        for d = 1:size(angal_i,2)
            ...
            simt_i = delta_zi ...
                    ./(soundspeed_targ_ana(2:size_speed) ...
                    .* sqrt(1-(p_c(d) .* soundspeed_targ_ana(2:size_speed)).^2));
    ...
    % 各层传播水平距离
    simy_i = (p_exm(i) .* soundspeed_targ_ana(2:size_speed) .* delta_zi)...
            ./(sqrt(1-(p_exm(i) .* soundspeed_targ_ana(2:size_speed)).^2));
...
% position = [0 0 z_target2]';% 参考点坐标轴原点
...
fprintf('初始估计: (%.2f, %.2f, %.2f) 误差: %.2f米\n', ...
    estimated_pos(pos_num,:)', norm(estimated_pos(pos_num,:)' - data_position'));
...
% boat_posreal_disp = [boat_position_real;boat_position_real(1,:)];
% scatter3(boat_pos_disp(:,1), boat_pos_disp(:,2), boat_pos_disp(:,3),'.','MarkerFaceColor','b');
plot(boat_pos_disp(:,1), boat_pos_disp(:,2), '.-','MarkerFaceColor','b', 'DisplayName', '观测点');
hold on;
% plot3(position_reme(:,1), position_reme(:,2), position_reme(:,3),'.--','MarkerFaceColor','y');
% legend('船舶测量位置','高斯牛顿迭代过程');
% scatter3(boat_posreal_disp(:,1), boat_posreal_disp(:,2), boat_posreal_disp(:,3), ...
